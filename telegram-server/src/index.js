...
// مسار داخلي لـ AI Command Center والجدولة فقط — لا يقبل JWT من المتصفح،
// بل سرّاً مشتركاً مع Supabase Edge Functions (نفس نمط whatsapp-server).
app.post('/api/internal/sessions/:channelId/send', async (req, res) => {
  if (!env.gatewaySecret || req.headers['x-gateway-secret'] !== env.gatewaySecret) {
  ...
  try {
    ...
    logError('internal-api', `فشل إرسال مجدول على ${channelId}`, error);
    res.status(500).json({ error: error instanceof Error ? error.message : 'خطأ غير متوقع' });
  }
});
